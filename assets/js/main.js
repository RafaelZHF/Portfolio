...
(function () {
  ...
  /* ------------------------------------------------------------------
     0b. Ambil versi teks terbaru dari Cloudflare Worker SEBELUM render.
         RL3_loadRemoteContent() (didefinisikan di content.js) mengisi
         ulang objek CONTENT dengan data dari KV kalau berhasil, atau
         diam-diam membiarkan CONTENT tetap berisi fallback default
         kalau gagal/timeout — jadi di sini renderContent() selalu aman
         dipanggil setelahnya apa pun hasilnya.

         Ini SATU-SATUNYA pemanggilan renderContent() untuk isi teks
         awal (tidak dipanggil dua kali) — beda dengan pendekatan
         "render fallback dulu, lalu render ulang kalau fetch selesai",
         supaya elemen yang di-innerHTML di renderContent() (link
         navbar, kolom footer, dst) hanya dibangun sekali dan event
         listener yang di-attach skrip lain setelahnya tidak pernah
         menempel ke elemen yang keburu diganti.

         typeof check untuk RL3_loadRemoteContent tetap dijaga sebagai
         jaring pengaman kalau suatu saat content.js dimuat versi lama
         tanpa fungsi ini (mis. cache browser agresif) — situs tetap
         jalan dengan fallback, cuma tanpa fitur sinkron Worker.
  ------------------------------------------------------------------ */
  const remoteContentReady =
    typeof RL3_loadRemoteContent === 'function' ? RL3_loadRemoteContent() : Promise.resolve();

  remoteContentReady.then(renderContent).catch(renderContent);

  /* ------------------------------------------------------------------
     1. Skeleton loading awal
        Skeleton ditahan tampil minimal SKELETON_MIN_DURATION supaya
        animasi shimmer-nya sempat terlihat jelas. Sekarang skeleton
        juga otomatis berfungsi sebagai "jeda tunggu" fetch /content di
        atas (fetch punya batas waktu 4 detik sendiri di content.js,
        jauh di bawah skeleton timeout 4 detik di bawah, jadi urutannya
        tetap wajar: data datang duluan, baru skeleton dilepas).
        Urutannya:
        1) tunggu window 'load' + durasi minimum + renderContent() selesai
  ...
  // Guard idempoten: revealRealContent() sekarang bisa terpanggil dari
  // dua jalur berbeda (alur normal Promise.all di atas, ATAU jaring
  // pengaman setTimeout 6 detik kalau alur normal macet) — tanpa guard
  // ini, event 'rl3:content-revealed' bisa ter-dispatch dua kali dan
  // membuat listener di skrip lain (mis. scroll-fx.js) jalan dobel.
  ...
  // windowLoadPromise + remoteContentReady digabung dengan Promise.all
  // supaya skeleton TIDAK pernah dilepas sebelum renderContent() selesai
  // memakai data hasil fetch (window 'load' pada halaman statis kecil
  // begini biasanya lebih cepat selesai daripada fetch /content ke
  // Worker, jadi kalau cuma mengandalkan 'load' saja, skeleton bisa
  // hilang duluan sebelum teks sempat berganti dari fallback ke data
  // Worker — sekilas terlihat seperti "kedapnya" konten berubah).
  const windowLoadPromise = new Promise((resolve) => {
    window.addEventListener('load', resolve, { once: true });
  });

  Promise.all([windowLoadPromise, remoteContentReady]).then(() => {
    setTimeout(revealRealContent, SKELETON_MIN_DURATION);
  });

  // Jaring pengaman: kalau salah satu promise di atas entah kenapa tidak
  // pernah selesai (mis. resource lambat/gagal, atau fetch tergantung
  // walau sudah ada AbortController), tetap tampilkan halaman setelah
  // waktu wajar supaya pengunjung tidak terjebak melihat skeleton
  // selamanya. Nilainya (6 detik) sengaja di atas timeout fetch di
  // content.js (4 detik) supaya jaring pengaman ini normalnya tidak
  // pernah lebih dulu terpicu daripada alur normal di atas.
