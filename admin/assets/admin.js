/* ==========================================================================
   ...
   TEKS dan FOTO tidak lagi disimpan sebagai file statis yang perlu commit
   — keduanya disimpan di Cloudflare KV lewat Worker yang sama dengan
   yang menangani login (lihat AUTH_API_BASE di bawah, dan
   worker/src/index.js untuk source Worker-nya). Alurnya sekarang:
     ...
     2. Begitu login sukses, dashboard mengambil DRAFT AWAL dari
        GET /content dan GET /photo (bukan lagi langsung memakai CONTENT
        bawaan content.js) — supaya kamu selalu mulai mengedit dari versi
        yang SEDANG LIVE di situs, bukan dari fallback statis yang bisa
        saja sudah usang. Kalau GET /content mengembalikan `content: null`
        (KV memang belum pernah diisi sama sekali), draft awal baru jatuh
        balik ke CONTENT bawaan.
     3. Dashboard menampilkan draft itu sebagai form, dikelompokkan
        persis seperti struktur section di content.js, ditambah satu
        section khusus Foto (lihat wireFotoField()).
     4. Setiap kali kamu ubah field, ada live preview jadi kamu bisa
        lihat hasilnya sebelum yakin, dan status "ada perubahan belum
     ...
     5. Klik "Simpan Perubahan" (bar bawah) ATAU "Apply Changes" (pojok
        ...
        mengirim PUT /content (dan PUT /photo kalau kamu ganti foto) ke
        Worker. Begitu berhasil, KV langsung ter-update. Lain kali situs
        utama (atau tab dashboard lain) di-refresh, GET /content otomatis
        mengembalikan versi baru ini — TIDAK ADA proses build/commit/push
        sama sekali, dan TIDAK ADA file yang perlu diunduh manual.
     6. Kalau berubah pikiran sebelum sempat Simpan, klik "Revert" — ini
        MEMBUANG seluruh draft yang sedang diedit dan mengambil ulang
        versi tersimpan terakhir dari server (GET /content & GET /photo),
   ...
   Lihat komentar di assets/js/content.js untuk detail lengkapnya.
   ========================================================================== */
...
(function () {
  ...
  // ==============================================================
  // SIMPAN — PUT /content (+ PUT /photo kalau ada foto baru)
  // ------------------------------------------------------------
