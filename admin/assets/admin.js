...
(function () {
  ...
  function init() {
    // "foto" adalah section pertama yang aktif di halaman UI Settings
    // (lihat currentUiSettingsSection) — set atribut ini dari awal
    // supaya CSS section-foto/section lain sudah benar sebelum data
    // apa pun sempat dimuat.
    document.body.setAttribute("data-ui-settings-section", "foto");
  ...
  // bagian.
  //
  // Section-section itu (15 key dari content.js) dikelompokkan ke
  // dalam GROUP yang punya arti (bukan cuma daftar rata satu level
  // seperti sebelumnya) lewat SECTION_GROUPS di bawah — supaya subnav
  // UI Settings punya struktur dua-level yang sama modelnya dengan
  // subnav utama di sidebar kiri (grup "Settings" berisi beberapa
  // halaman). Tiap section juga cuma diberi LABEL BERSIH (tanpa
  // prefix "01 —" dst.) — nomor urut dari content.js (mis.
  // eyebrowIndex, chapterNum) tetap tampil sebagai FIELD biasa di
  // dalam form, bukan digabung paksa ke nama section di navigasi.
  //
  // PENTING soal render: hanya SATU section yang benar-benar
  // di-render ke DOM dalam satu waktu (lihat renderActiveSection()),
  // bukan ke-15 nya sekaligus ditumpuk seperti sebelumnya. Pindah
  // section dari subnav = ganti section mana yang di-render, mirip
  // persis pola navigateToPage() untuk halaman Overview/UI Settings/
  // dst. Field values tetap dibaca/ditulis langsung ke draftContent
  // (lihat renderPrimitiveField), jadi berpindah section tidak
  // pernah kehilangan perubahan yang belum disimpan di section lain.
  // ==============================================================
  const SECTION_LABELS = {
    meta: "Meta Browser & SEO",
    navbar: "Navbar",
    mobileDrawer: "Menu Mobile",
    hero: "Hero",
    logoBar: "Logo Bar Berjalan",
  ...
  // Urutan grup = urutan tampil di subnav. Urutan section di dalam
  // tiap grup = urutan tampil di dalam grup itu.
  const SECTION_GROUPS = [
    ...
    },
    {
      id: "navigasi",
      label: "Navigasi",
      sections: ["navbar", "mobileDrawer"],
    },
    {
      id: "hero-marquee",
      label: "Hero & Logo Bar",
      sections: ["hero", "logoBar"],
    },
    {
      id: "konten-halaman",
      label: "Konten Halaman",
      sections: ["tentang", "genre", "skill", "karya", "alat", "statistik", "quote", "motto"],
    },
    {
      id: "kontak-footer",
      label: "Kontak & Footer",
      sections: ["kontak", "footer"],
    },
  ];

  const SECTION_ORDER = SECTION_GROUPS.reduce((acc, group) => acc.concat(group.sections), []);

  // Section aktif saat ini di dalam halaman UI Settings (bukan
  // currentPage — itu untuk 4 halaman dashboard di level atas).
  // "foto" adalah section semu (bukan dari content.js, lihat section
  // Foto yang statis di admin/index.html).
  let currentUiSettingsSection = "foto";
  ...
  function buildFormFromContent(content) {
    // draftContent dipegang di sini (bukan cuma sebagai parameter)
    // supaya renderActiveSection() yang dipanggil belakangan (mis.
    // saat pindah section lewat subnav) selalu baca data TERBARU,
    // bukan snapshot lama dari saat buildFormFromContent() pertama
    // dipanggil.
    ...
    // Section semu "foto" selalu jadi default begitu form dibangun
    // ulang (login baru / revert) — sama seperti navigateToPage()
    // yang selalu balik ke "overview" saat login baru. Set atribut
    // body di sini juga (bukan cuma lewat switchUiSettingsSection())
    // karena ini bisa jadi RESET dari section lain yang sebelumnya
    // aktif (mis. revert saat sedang di section "Navbar") — atribut
    // body harus ikut balik ke "foto", bukan cuma state JS-nya saja.
    currentUiSettingsSection = "foto";
    document.body.setAttribute("data-ui-settings-section", "foto");
    renderActiveSection();
    refreshPreview();
  }

  // Render HANYA section yang sedang aktif ke #formSections — bukan
  // seluruh 15 section content.js sekaligus. Dipanggil setiap kali
  // currentUiSettingsSection berubah (klik subnav) maupun setiap kali
  // draftContent di-assign ulang (buildFormFromContent()).
  function renderActiveSection() {
    const container = document.getElementById("formSections");
    if (!container) return;
    container.innerHTML = "";

    if (currentUiSettingsSection === "foto") {
      // Section Foto ditulis statis di admin/index.html (di luar
      // #formSections, lihat #section-foto) — di sini cukup pastikan
      // section itu yang terlihat kalau memang ada.
      return;
    }

    const sectionKey = currentUiSettingsSection;
    const sectionValue = draftContent ? draftContent[sectionKey] : null;
    if (!sectionValue) return;

    const sectionEl = renderSection(sectionKey, sectionValue);
  ...
  // Pindah section aktif di dalam halaman UI Settings (dipanggil dari
  // klik subnav, lihat buildUiSettingsSubnav()). Beda dengan
  // navigateToPage() di level atas — ini TIDAK mengubah location.hash,
  // murni state internal halaman UI Settings.
  function switchUiSettingsSection(sectionKey) {
    if (sectionKey === currentUiSettingsSection) return;
    currentUiSettingsSection = sectionKey;

    renderActiveSection();

    // Section Foto statis di index.html (bukan di #formSections) —
    // saat aktif, sembunyikan #formSections supaya tidak ada area
    // form kosong yang menggantung; saat section lain aktif,
    // sebaliknya. Section Foto sendiri diatur tampil/sembunyi lewat
    // CSS berdasarkan atribut ini di elemen <body>.
    document.body.setAttribute("data-ui-settings-section", sectionKey);

    // Balik scroll kolom form ke atas setiap ganti section — supaya
    // "buka Navbar" selalu mulai dari field pertama Navbar, bukan
    // meneruskan posisi scroll dari section sebelumnya.
    const formCol = document.querySelector(".settings-form-col");
  ...
  function renderArrayField(parentEl, arr, path) {
    ...
    arr.forEach((item, index) => {
      ...
      if (item !== null && typeof item === "object") {
        ...
        // Header kartu = badge nomor bulat (bukan lagi "#1 — Nama"
        // digabung dengan garis strip panjang) + nama item, sebagai
        // dua elemen terpisah supaya nomor urut bisa dibuat lebih
        // kecil dan tidak "menyatu" dengan teks nama.
        const cardHeader = document.createElement("div");
        cardHeader.className = "array-item-header";

        const cardBadge = document.createElement("span");
        cardBadge.className = "array-item-badge";
        cardBadge.textContent = String(index + 1).padStart(2, "0");
        cardHeader.appendChild(cardBadge);

        const cardName = describeArrayItemName(item);
        if (cardName) {
  ...
  // Nama item untuk header kartu array (mis. "Tentang" untuk link
  // navbar pertama) — hanya nama-nya saja, TANPA nomor urut digabung
  // (nomor sudah tampil terpisah lewat .array-item-badge). Balik null
  // kalau item tidak punya field nama yang cocok (badge nomor saja
  // sudah cukup identitas untuk kartu itu).
  function describeArrayItemName(item) {
  ...
  // ==============================================================
  // SUBNAV UI SETTINGS (pindah antar section di dalam halaman ini)
  // ------------------------------------------------------------
  // Subnav dua-level: GRUP (mis. "Konten Halaman") berisi beberapa
  // SECTION (mis. Tentang Saya, Genre, Keahlian, dst) — modelnya
  // sama dengan grup "Settings" yang bisa diciutkan di sidebar utama
  // kiri (lihat .side-group-collapsible di admin/index.html).
  //
  // Beda penting dari versi sebelumnya: klik item di sini TIDAK
  // scrollIntoView ke section yang sudah ada semua di DOM — sekarang
  // memanggil switchUiSettingsSection() yang GANTI section mana yang
  // di-render (lihat renderActiveSection()). Jadi buka "Navbar" cuma
  // menampilkan field Navbar, section lain (mis. "Menu Mobile") sama
  // sekali tidak ikut di-render sampai memang diklik.
  //
  // Section Foto (dari admin/index.html, bukan content.js) tetap jadi
  // entri pertama, di luar grup mana pun — sengaja begitu karena foto
  // memang bukan bagian dari struktur content.js.
  // ==============================================================
  function buildUiSettingsSubnav(orderedKeys) {
    ...
    // ---- Entri "Foto", berdiri sendiri di luar grup ----
    const fotoLink = document.createElement("button");
    fotoLink.type = "button";
    fotoLink.className = "form-nav-link is-active";
    fotoLink.textContent = "Foto (Avatar Chat)";
    fotoLink.addEventListener("click", () => {
      switchUiSettingsSection("foto");
      nav.querySelectorAll(".form-nav-link").forEach((l) => l.classList.remove("is-active"));
      fotoLink.classList.add("is-active");
    });
    nav.appendChild(fotoLink);

    const divider = document.createElement("div");
    divider.className = "form-nav-divider";
    nav.appendChild(divider);

    // ---- Sisanya dikelompokkan per SECTION_GROUPS ----
    // orderedKeys dipakai untuk memfilter: kalau suatu saat ada key
    // baru di content.js yang belum terdaftar di SECTION_GROUPS,
    // tetap muncul (dikelompokkan ke grup "Lainnya") alih-alih hilang
    // diam-diam dari subnav.
    const groupedKeys = new Set();
    SECTION_GROUPS.forEach((g) => g.sections.forEach((k) => groupedKeys.add(k)));
    const ungroupedKeys = orderedKeys.filter((k) => !groupedKeys.has(k));

    const allGroups = ungroupedKeys.length
      ? SECTION_GROUPS.concat([{ id: "lainnya", label: "Lainnya", sections: ungroupedKeys }])
      : SECTION_GROUPS;

    allGroups.forEach((group) => {
      const sectionsInThisGroup = group.sections.filter((k) => orderedKeys.indexOf(k) !== -1);
      if (!sectionsInThisGroup.length) return;

      const groupLabel = document.createElement("p");
      groupLabel.className = "form-nav-group-label";
      groupLabel.textContent = group.label;
      nav.appendChild(groupLabel);

      sectionsInThisGroup.forEach((key) => {
        const link = document.createElement("button");
        link.type = "button";
        link.className = "form-nav-link";
        link.textContent = SECTION_LABELS[key] || humanizeKey(key);
        link.addEventListener("click", () => {
          switchUiSettingsSection(key);
          nav.querySelectorAll(".form-nav-link").forEach((l) => l.classList.remove("is-active"));
